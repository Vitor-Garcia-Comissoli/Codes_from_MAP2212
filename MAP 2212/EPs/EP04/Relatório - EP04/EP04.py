...
class Estimador:
    ...
    def __init__(self,x,y):
        ...
        self.n = 300000 # N decidido para um erro < 0.0005
        self.k = self.n // 2 # número de bins
        
        theta = dirichlet(alpha, self.n) # chama a função dirichlet
...
def f_dirichlet(alpha, theta):
    return dr.pdf(theta.T, alpha)

def gera_v(k, f_theta):
    dim_theta = f_theta.shape[0]
    extra = k - dim_theta % k                       # numero de zeros a adicionar ao array se dimencao de theta nao e divisivel por k
    if extra == k:
        extra = 0
    ordenado_completo = np.zeros(dim_theta + extra) # array de tamanho multiplo de k
    
    ordenado = np.sort(f_theta)                     # ordena os valores da f(theta)
    ordenado_completo[:dim_theta] = ordenado        # ultimo bin possivelmente tem zeros adicionais, mas queremos apenas o max
...
def encontra_n():
    ...
    estimativa.n = 2500000

    anterior = 0

    nova_est = estimativa.U(25)
    while anterior == 0 or abs(anterior - nova_est) / anterior > 0.0005:
        estimativa.n += 100000
        estimativa.k = estimativa.n // 2
        theta = dirichlet(estimativa.vetor_x + estimativa.vetor_y, estimativa.n) # chama a função dirichlet
        f_theta = f_dirichlet(estimativa.vetor_x + estimativa.vetor_y, theta) # chama a função f_dirichlet
        
        estimativa.v_hat = gera_v(estimativa.k, f_theta) # chama a função gera_v
